...
const { dbModels, User, Product } = require('./model');
const hooks = require('./hooks');
const { sequelize, DataTypes, Sequelize, Model } = require('./sequelize');


app.get('/', (req, res, next) => {
    //initialize the history table for models
    const attrs = Object.keys(Product.rawAttributes).filter(key => {
        if (key !== 'createdAt' && key !== 'updatedAt' && key !== 'deletedAt') {
            return key;
        }
    });
    const values = [];
    Object.entries(Product.rawAttributes).filter(([key, value]) => {
        if (key !== 'createdAt' && key !== 'updatedAt' && key !== 'deletedAt') {
            values.push(Product.rawAttributes[key]);
        }
    });
    let body = {};
    for (let i = 0; i < attrs.length; i++) {
        Object.assign(body, { [attrs[i]]: values[i] });
    }
    // console.log(body);

    dbModels.forEach(p => {
        sequelize.define(p.getTableName() + 'history', {
            ...body,
            ip: { type: DataTypes.STRING },
            opration: { type: DataTypes.STRING },
            platform: { type: DataTypes.STRING },
            updatedAt: { type: DataTypes.DATE },
            restoredAt: { type: DataTypes.DATE }
        }, { freezeTableName: true }).sync({ alter: true });
    });

    User.addHook('beforeUpdate','userHistory',(user,options)=>{console.log(Sequelize)})

    new hooks(req, User, { fullRow: false }).throwHook();
    // new hooks(req, Product, {  fullRow: false }).throwHook();
    ...
    update1(User, 1, { firstName: 'aabbbaaar' });
    ...
    // undoUpdate(User, UserHistory, 1, 2);
